...
- If titles of other videos in the same playlist are provided, they take precedence over the rest of the library: match their series name, numbering format and separators, and treat the library list only as secondary context
...
export async function buildTitleUserPrompt(metadata: MetadataStore, filename: string, playlistTitles: string[]): Promise<string> {
  const existing = await metadata.list();
  const titles = existing.map((v) => v.title);
  const playlistSection = playlistTitles.length > 0
    ? `Titles of the other videos in the same playlist as this one (primary reference — follow their naming pattern):
    ${playlistTitles.map((t) => `- ${t}`).join('\n')}
    `
    : '';
  return `Filename: ${filename}.
    ${playlistSection}Existing video titles in the library:

...
describe('getVideoFile manifest rewriting', () => {
  it('rewrites .ts segment lines to presigned URLs and keeps comments', async () => {
    ...
    if (file?.status === 'ready' && file.kind === 'manifest') {
      ...
      expect(file.body).toContain('https://s3.test/videos/');
      expect(file.body).not.toContain('index_hls_00001.ts\n');
    } else {
      throw new Error('expected manifest');
    }
  });

  it('redirects a direct segment request to a presigned URL', async () => {
    const metadata = await readyVideo();

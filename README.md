...
#### 現在利用可能な環境変数
- `PORT` - バックエンドサーバーポート（デフォルト: 4000）
- `NODE_ENV` - 環境モード（development/production、デフォルトなし）

#### ハードコードされている値（将来的に設定可能にすべき）
- フロントエンドポート: 3000
- アップロード制限: 5GB
- HLSセグメント期間: 10秒
- サムネイルタイムスタンプ: 10秒
- 動画の保存先ファイルパス: backend/videos
